...
func (t *Tree) newList(start Pos) *ListNode {
	// Initialize length with -1, it will be updated when nodes are added.
...
func (t *Tree) newPipeline(startOffset Pos, line int, vars []*VariableNode) *PipeNode {
	// Initialize length with -1, it will be updated when child nodes are added.
...
type CommandNode struct {
	NodeType
	Location
	Value string // TODO (PT): is it needed or did I forget to remove it after experiments?
	tr    *Tree
...
func (c *CommandNode) append(arg Node) {
	...
	// update command's length, including both existing node, arg and space between them
	c.length = int(arg.StartOffset()-c.StartOffset()) + arg.Length()
}
...
func (t *Tree) newChain(node Node) *ChainNode {
	// Initialize location from `node`, it will be updated when fields are added.
	return &ChainNode{tr: t, NodeType: NodeChain, Location: Location{node.StartOffset(), node.Length()}, Node: node}
}
